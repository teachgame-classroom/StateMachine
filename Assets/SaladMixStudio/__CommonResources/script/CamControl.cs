...
public class CamControl : MonoBehaviour {
	...
	void LateUpdate() {	
		...
		if ((target.position.x - ground.position.x) >= 10f) {
			groundPos.x += 10f;
			ground.position = groundPos;
		}
		else if((target.position.x - ground.position.x) <= -10f){
			groundPos.x -= 10f;
			ground.position = groundPos;
		}
		if((target.position.z - ground.position.z) >= 10f){
			groundPos.z += 10f;
			ground.position = groundPos;
		}
		else if((target.position.z - ground.position.z) <= -10f){
			groundPos.z -= 10f;
			ground.position = groundPos;
		}
